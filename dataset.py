...
class BlockDataset(data.Dataset):
    def __init__(self,
        ...
        uns_rimg = nn.functional.interpolate(uns_rimg, scale_factor=rescale_factor, mode="trilinear",
                                             align_corners=False)
        rimg = torch.squeeze(uns_rimg, 0)

        if isinstance(bfld, torch.Tensor):
            uns_bfld = torch.unsqueeze(bfld, 0)
            uns_bfld = nn.functional.interpolate(uns_bfld, scale_factor=rescale_factor, mode="trilinear",
                                                 align_corners=False)
